class @EditEvent extends @NewEvent
  ...
  setDeleteRoute: =>
    if @$editTypeRadio.length == 0
      if @occurrence.occurrence_type == "singular"
  ...
  setSingularDeleteRoute: =>
    @$form.data @deleteActionAttributeName, Routing.generate("_events_delete_singular",{occurrence:@occurrence.id})
    
  setSerialDeleteRoute: =>
    @$form.data @deleteActionAttributeName, Routing.generate("_events_delete_serial",{occurrence:@occurrence.id})
  ...
  handleEditTypeSwitch: (e) =>
    ...
    if @$editTypeRadio.filter(":checked").val() == "singular" or @$editTypeRadio.filter(":checked").length == 0
      @$startDate.val moment(@occurrence.start_date).format("DD.MM.YYYY HH:mm")
      @descriptionDiv.show()
      @daysDiv.hide()
      @eventTypeDiv.hide()
      @endDateDiv.hide()
