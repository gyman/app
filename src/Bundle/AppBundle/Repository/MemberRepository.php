...
class MemberRepository extends EntityRepository implements MemberRepositoryInterface
{
    ...
    public function search(SearchMemberCommand $command)
    {
        ...
        if (null !== $command->hasOpenedEntry) {
            $qb->join('m.lastEntry', 'e');
            $column = 'e.endDate';
            $qb->andWhere($expr->andX(
                (bool) ($command->hasOpenedEntry) ? $expr->isNull($column) : $expr->isNotNull($column)
            ));
        }

        if (null !== $command->hasVoucher) {
            $qb->andWhere(
                (bool) ($command->hasVoucher) ? $expr->isNotNull('m.currentVoucher') : $expr->isNull('m.currentVoucher')
            );
        }

        if (null !== $command->starred) {
            $qb->andWhere(
                $expr->eq('m.details.starred', true)
            );
        }

        if (null !== $command->belt) {
